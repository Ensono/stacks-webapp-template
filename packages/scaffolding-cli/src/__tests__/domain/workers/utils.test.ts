...
const mock_vals: Array<Replacetruct> = [
    {
        replaceFiles: ["/some/dir/test-app-1/**/*.md"],
        replaceVals: {from: "foo", to: "test-app-1"},
    },
]
...
describe("utils class tests", () => {
    ...
    describe("Positive assertions", () => {
        it("copyWorker should return success", async () => {
            const copy_ran: TempCopy = await Utils.prepBase(
                mock_answer.projectName,
            )
        ...
        it("valueReplace should return ok", async () => {
            const replace_ran: BaseResponse = await Utils.valueReplace(
                mock_vals,
            )
        ...
        it("doGitClone should return success", async () => {
            ...
            expect(git_ran.message).toMatch(
                "Git Cloned from repo and checked out on specified head",
            )
        })
    ...
    describe("Negative assertions", () => {
        ...
        it("writeOutConfigFile should return a code of ENOENT when error occurs", async () => {
            ...
            try {
                await Utils.writeOutConfigFile(
                    "/tmp/foo.json",
                    mock_cli_answer_model,
                )
            } catch (ex) {
