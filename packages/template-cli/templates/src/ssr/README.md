...
1. How are defining the routing for API's?
...
- To maximize cache layer capacity we should copy over package.json into /tmp
  and build there
...
A global 'provider' variable is setup in the [pactSetup.ts](./pact/pactSetup.ts)
file. Then the [pactTestWrapper.ts](./pact/pactTestWrapper.ts) ensures each test
...
file, there is a [pactPublish](./pact/pactPublish.ts) script to publish the
