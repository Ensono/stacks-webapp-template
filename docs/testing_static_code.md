...
SonarCloud collects the coverage reports generated by Jest in the form of `.lcov` output. Configuration in the `jest.config.json` file will include the following for code coverage collection:
...
The coverage directory will be where the .lcov` reports are generated. T
